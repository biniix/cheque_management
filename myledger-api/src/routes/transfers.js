...
router.post('/', async (req, res) => {
  ...
  try {
    const { account_id, customer_id, amount, date, description = '' } = req.body;

    const [accounts] = await conn.query('SELECT * FROM accounts WHERE id = ? AND user_id = ?', [account_id, req.userId]);
    ...
    let customerName = '';
    if (customer_id) {
      const [customers] = await conn.query('SELECT * FROM customers WHERE id = ? AND user_id = ?', [customer_id, req.userId]);
    ...
    const [txnResult] = await conn.query(
      'INSERT INTO transactions (account_id, customer_id, type, amount, date, reference_no, description, payee) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      [account_id, customer_id || null, 'transfer', -amount, date, refNo, description, customerName]
    );
