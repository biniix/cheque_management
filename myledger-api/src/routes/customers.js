...
router.get('/', async (req, res) => {
  try {
    const customers = await db.all('SELECT * FROM customers WHERE user_id = ?', [req.userId]);
...
router.put('/:id', async (req, res) => {
  try {
    const customer = await db.get('SELECT * FROM customers WHERE id = ? AND user_id = ?', [req.params.id, req.userId]);
