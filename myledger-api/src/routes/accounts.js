...
router.get('/', async (req, res) => {
  try {
    const accounts = await db.all('SELECT * FROM accounts WHERE user_id = ?', [req.userId]);
...
router.post('/:id/deposit', async (req, res) => {
  ...
  try {
    ...
    const [accounts] = await conn.query('SELECT * FROM accounts WHERE id = ? AND user_id = ?', [accountId, req.userId]);
    ...
    const [txnResult] = await conn.query(
      ...
      [accountId, 'deposit', amount, date, refNo, description]
    );
    ...
    const [rcptResult] = await conn.query(
      'INSERT INTO receipts (transaction_id, reference_no) VALUES (?, ?)',
      [txnId, receiptRef]
    );
